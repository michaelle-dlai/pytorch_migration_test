...
def part_4(
    ...
    @object_to_grade(learner_mod, "prepare_qat")
...
def part_3(
    ...
    @object_to_grade(learner_mod, "fuse_model_inplace")
    def g(learner_func: FunctionType) -> List[test_case]:
        cases = []
...
def part_2(
    ...
    @object_to_grade(learner_mod, "quantize_dynamic_linear")
    def g(learner_func: FunctionType) -> List[test_case]:
        cases = []
                ...
                def _probe_dtype(mod):
                    ...
                    if pp is not None:
                        for attr in ("dtype", "weight_dtype", "_dtype", "_weight_dtype"):
                            ...
                            if dt is not None:
                                return dt
                    # sniff repr() for qint8/quint8 hints
                    r = repr(mod)
                    if "qint8" in r:
                        return torch.qint8
                    if "quint8" in r:
...
def part_1(
    ...
    @object_to_grade(learner_mod, "prune_model")
    def g(learner_func: FunctionType) -> List[test_case]:
        cases = []
        # === function type check ===
        t = test_case()
